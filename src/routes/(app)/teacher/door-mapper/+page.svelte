<script lang="ts">
  ...
  function drawLine(door: {room1_id: number, room2_id: number}, firstRoomCenter?: DOMPoint, secondRoomCenter?: DOMPoint) {
    if (firstRoomCenter && secondRoomCenter) {
      const line = map.getSVG().appendChild(document.createElementNS(SVG_NS, "line"));
